<template>
  <v-container>
    <v-data-table-server :headers="headers" :items="items" :items-length="pagination.totalItems" :loading="loading"
      v-model:items-per-page="pagination.itemsPerPage" v-model:page="pagination.page" @update:options="getOffices">
      <template v-slot:top>
        <v-toolbar flat>
          <v-toolbar-title>Offices</v-toolbar-title>
          <v-divider class="mx-2" inset vertical></v-divider>
          <v-spacer></v-spacer>
          <v-text-field v-model="search" append-icon="mdi-magnify" label="Search" class="me-4" single-line hide-details
            @change="getOffices"></v-text-field>
          <v-btn class="btn border text-light bg-dark px-2 rounded me-4" :to="{ name: 'AdminOfficesCreate' }">
            Create Office
          </v-btn>
        </v-toolbar>
      </template>
      ...
      <template v-slot:item.actions="{ item }">
        <div class="d-flex">
          ...
          <v-btn v-if="userType === 'admin'" icon @click="deleteOffice(item)" class="mx-2">
